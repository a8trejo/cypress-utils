...
class SeleniumEasy {
    ...
    alertsAndModals = (webElement: string) => cy.get('.tree-branch ul .tree-branch').contains(webElement)
    javascriptAlerts = (webElement: string) =>
        cy.get(".tree-branch ul .tree-branch li[style='display: list-item;']").contains(webElement)
...
export const seleniumEasyPage = new SeleniumEasy()
