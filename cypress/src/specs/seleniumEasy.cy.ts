...
describe('Selenium Easy Web Automation', { browser: 'electron' }, () => {
    testFixtures['Form Data'].forEach((plot: any) => {
        it(`Dynamic Test Case Input Form: ${plot.title}`, () => {
            if ('tags' in plot) {
                testMeta.tags = plot.tags
            }
            cy.visit('/input-form-demo.html')
            seleniumEasyPage.firstNameInput().should('be.visible').and('be.enabled').type(plot.arguments.firstName)
    ...
    it('Connect to MySQL DB and execute query', () => {
        ...
        cy.task('dbExec', { dbName: 'wordpress-db', sql: sqlQuery }).then((sqlRows: any) => {
